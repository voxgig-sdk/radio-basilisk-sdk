...
import (
	...
	omni "github.com/voxgig-sdk/radio-basilisk-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RadioBasiliskError); is {
				out := map[string]any{
					"name":    "RadioBasiliskError",
...
func omniCtx(arg any, client *sdk.RadioBasiliskSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RadioBasiliskSDK); is {
			client = live
