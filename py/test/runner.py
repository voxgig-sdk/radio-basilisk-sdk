# RadioBasilisk SDK test runner

from __future__ import annotations
import os
import json

from radiobasilisk_sdk.utility.voxgig_struct import voxgig_struct as vs


class RadioBasiliskTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RadioBasiliskTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RadioBasiliskTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RadioBasiliskTestRunner.getenv("RADIO_BASILISK_TEST_LIVE")
        override = RadioBasiliskTestRunner.getenv("RADIO_BASILISK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RadioBasiliskTestRunner.getenv(key)
        ...
        explain = RadioBasiliskTestRunner.getenv("RADIO_BASILISK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["RADIO_BASILISK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if RadioBasiliskTestRunner._test_control is not None:
            return RadioBasiliskTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RadioBasiliskTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RadioBasiliskTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RadioBasiliskTestRunner.load_test_control()
...
def load_env_local():
    RadioBasiliskTestRunner.load_env_local()


def env_override(m):
    return RadioBasiliskTestRunner.env_override(m)


def entity_data(v):
    return RadioBasiliskTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RadioBasiliskTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RadioBasiliskTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RadioBasiliskTestRunner.load_test_control()


def live_client_options():
    return RadioBasiliskTestRunner.live_client_options()


def live_delay_ms():
    return RadioBasiliskTestRunner.live_delay_ms()
