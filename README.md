...
Radio Basilisk provides daily information and entertainment to 100,000 listeners in the Basel region.

Learn more about Radio Basilisk API at [basilisk.ch](https://basilisk.ch).
...
$musics = $client->Music()->list();
print_r(array_map(fn($item) => $item->data_get(), $musics));
```
