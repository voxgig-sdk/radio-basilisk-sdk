// Command RadioBasilisk-cli is a CLI + REPL for the RadioBasilisk SDK,
...
const prompt = "radio-basilisk"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "music"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      RADIO_BASILISK_APIKEY (api key), RADIO_BASILISK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: RADIO_BASILISK_APIKEY carries the API key and
	// RADIO_BASILISK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("RADIO_BASILISK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("RADIO_BASILISK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewRadioBasiliskSDK(opts)
