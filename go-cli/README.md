...
boru-driven command-line client **and** interactive REPL for the RadioBasilisk
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radio-basilisk-cli)
make build

# 2. See usage (words, entities, env vars)
./radio-basilisk-cli --help

# 3. Provide credentials once, via the environment
export RADIO_BASILISK_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./radio-basilisk-cli list music

# 5. Override the API base URL for a single call
RADIO_BASILISK_BASE=https://api.example.com ./radio-basilisk-cli list music

# 6. No arguments -> interactive REPL
./radio-basilisk-cli
radio-basilisk> list music
radio-basilisk> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radio-basilisk-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/radio-basilisk-cli list music
   ```
...
Run with no arguments to open a REPL (prompt `radio-basilisk>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `music`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/radio-basilisk-cli`. |
...
(`./radio-basilisk-cli <expr>`) and interactively (the REPL), and expressions compose the
