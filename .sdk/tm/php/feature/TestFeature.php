...
// RadioBasilisk SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class RadioBasiliskTestFeature extends RadioBasiliskBaseFeature
{
    ...
    public function init(RadioBasiliskContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (RadioBasiliskContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = RadioBasiliskParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (RadioBasiliskContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(RadioBasiliskContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = RadioBasiliskParam::call($ctx, $k);
