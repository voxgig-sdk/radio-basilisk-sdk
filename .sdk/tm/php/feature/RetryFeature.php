...
// RadioBasilisk SDK retry feature
...
class RadioBasiliskRetryFeature extends RadioBasiliskBaseFeature
{
    ...
    public function init(RadioBasiliskContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RadioBasiliskContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RadioBasiliskContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RadioBasiliskContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
