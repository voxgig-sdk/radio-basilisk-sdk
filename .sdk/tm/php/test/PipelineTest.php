...
// RadioBasilisk SDK pipeline test
//
...
{
    private static function utility(): RadioBasiliskUtility
    {
        return new RadioBasiliskUtility();
    }
    ...
    private static function ctx(array $over = []): RadioBasiliskContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new RadioBasiliskContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new RadioBasiliskOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof RadioBasiliskError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new RadioBasiliskOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = RadioBasiliskMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new RadioBasiliskOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = RadioBasiliskMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new RadioBasiliskOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = RadioBasiliskMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RadioBasiliskSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = RadioBasiliskMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new RadioBasiliskResponse([]),
            'result' => new RadioBasiliskResult([]),
        ]);
        [, $err] = RadioBasiliskMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec([]),
            'response' => null,
            'result' => new RadioBasiliskResult([]),
        ]);
        [, $err] = RadioBasiliskMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec(['step' => 's']),
            'response' => new RadioBasiliskResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new RadioBasiliskResult([]),
        ]);
        [, $err] = RadioBasiliskMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec(['step' => 's']),
            'response' => new RadioBasiliskResponse(self::resp(200, ['v' => 1])),
            'result' => new RadioBasiliskResult([]),
        ]);
        [, $err] = RadioBasiliskMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec(['step' => 's']),
            'response' => new RadioBasiliskResponse(self::resp(200, ['v' => 2])),
            'result' => new RadioBasiliskResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        RadioBasiliskMakeResponse::call($ctx);
    ...
    {
        $preset = new RadioBasiliskResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec([]),
            'response' => new RadioBasiliskResponse([]),
            'result' => new RadioBasiliskResult([]),
        ...
        [$got, $err] = RadioBasiliskMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new RadioBasiliskResult([])]);
        [, $err] = RadioBasiliskMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new RadioBasiliskSpec([]), 'result' => null]);
        [, $err] = RadioBasiliskMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RadioBasiliskSpec(['step' => 's']),
            'result' => new RadioBasiliskResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new RadioBasiliskOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RadioBasiliskMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new RadioBasiliskSpec(['step' => 's']),
            'result' => new RadioBasiliskResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new RadioBasiliskOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = RadioBasiliskMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new RadioBasiliskResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new RadioBasiliskSpec([]),
            'result' => new RadioBasiliskResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = RadioBasiliskMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RadioBasiliskMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new RadioBasiliskError('boom', 'boom');
        $utility->fetcher = function (RadioBasiliskContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RadioBasiliskSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = RadioBasiliskMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (RadioBasiliskContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = RadioBasiliskMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(RadioBasiliskResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        RadioBasiliskMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (RadioBasiliskContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new RadioBasiliskSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = RadioBasiliskMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new RadioBasiliskResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new RadioBasiliskSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = RadioBasiliskMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = RadioBasiliskMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = RadioBasiliskMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new RadioBasiliskResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, RadioBasiliskDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new RadioBasiliskResult(['ok' => false])]);
        $this->expectException(RadioBasiliskError::class);
        RadioBasiliskDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new RadioBasiliskResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', RadioBasiliskMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        RadioBasiliskMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new RadioBasiliskBaseFeature();
        $b = new RadioBasiliskBaseFeature();
        RadioBasiliskFeatureAdd::call($ctx, $a);
        RadioBasiliskFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): RadioBasiliskBaseFeature
    {
        $f = new RadioBasiliskBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        RadioBasiliskFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): RadioBasiliskContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new RadioBasiliskSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = RadioBasiliskPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        RadioBasiliskPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new RadioBasiliskResponse(['headers' => null]),
            'result' => new RadioBasiliskResult([]),
        ]);
        RadioBasiliskResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        RadioBasiliskResultBody::call($ctx);
