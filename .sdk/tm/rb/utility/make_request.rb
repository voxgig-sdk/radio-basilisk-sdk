...
module RadioBasiliskUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RadioBasiliskResponse.new({})
    ...
    if fetch_err
      ...
      response = RadioBasiliskResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RadioBasiliskResponse.new(fetched)
    else
