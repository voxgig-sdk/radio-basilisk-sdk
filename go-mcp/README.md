...
[MCP](https://modelcontextprotocol.io) server exposing the RadioBasilisk SDK as
two agent tools — `radio-basilisk_list` and `radio-basilisk_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/radio-basilisk-mcp)
make build

# 2. Provide credentials via the environment
export RADIO_BASILISK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user radio-basilisk \
  -- /absolute/path/to/radio-basilisk-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./radio-basilisk-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// radio-basilisk_list: first page of records
{ "entity": "music" }
{ "entity": "music", "query": { } }

// radio-basilisk_load: one record by id
{ "entity": "music", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/radio-basilisk-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user radio-basilisk \
     -- "$PWD"/dist/*/radio-basilisk-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `radio-basilisk_list` and `radio-basilisk_load` tools now appear
   in new sessions. Ask the agent to *"list music using radio-basilisk"*
   and it calls `radio-basilisk_list` with `{"entity":"music"}`.
...
```sh
export RADIO_BASILISK_APIKEY=sk_live_xxx            # API key
export RADIO_BASILISK_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `radio-basilisk_list` tool
...
### Call the `radio-basilisk_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `radio-basilisk_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `radio-basilisk_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RADIO_BASILISK_APIKEY` | API key sent with every request. |
| `RADIO_BASILISK_BASE` | Optional override of the API base URL. |
...
```sh
./radio-basilisk-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"radio-basilisk_load","arguments":{"entity":"music","query":{"id":1}}}}'
```
