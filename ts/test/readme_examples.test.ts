...
//     (\`new RadioBasiliskSDK(...)\` / \`RadioBasiliskSDK.test(...)\` become
//     \`RadioBasiliskSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RadioBasiliskSDK'
...
const TEST_SEED = {"entity":{"music":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new RadioBasiliskSDK(...)\` and any
// \`RadioBasiliskSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RadioBasiliskSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RadioBasiliskSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
