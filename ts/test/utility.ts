...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.RADIOBASILISK_TEST_LIVE ||
    'TRUE' === process.env.RADIOBASILISK_TEST_OVERRIDE
  ) {
  ...
  m.RADIOBASILISK_TEST_EXPLAIN = process.env.RADIOBASILISK_TEST_EXPLAIN || m.RADIOBASILISK_TEST_EXPLAIN
