...
class MusicEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = RadioBasiliskSDK.test(seed, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RADIO_BASILISK_TEST_MUSIC_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def music_basic_setup(extra)
  ...
  entid_env_raw = ENV["RADIO_BASILISK_TEST_MUSIC_ENTID"]
  ...
  if env["RADIO_BASILISK_TEST_LIVE"] == "TRUE"
    ...
    client = RadioBasiliskSDK.new(Helpers.to_map(merged_opts))
  end
