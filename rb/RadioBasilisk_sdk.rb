...
require_relative 'RadioBasilisk_types'
...
class RadioBasiliskSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RadioBasiliskHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RadioBasiliskHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RadioBasiliskFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RadioBasiliskUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RadioBasiliskHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RadioBasiliskHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RadioBasiliskHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RadioBasiliskError.new(
        "#{op}_allow",
        "RadioBasiliskSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RadioBasiliskHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RadioBasiliskError.new(
        "graphql_error", "RadioBasiliskSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RadioBasiliskSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
