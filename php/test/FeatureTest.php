...
// RadioBasilisk SDK feature test
//
...
class FtCtrl extends RadioBasiliskControl
{
...
class FtClient
{
    ...
    private RadioBasiliskUtility $utility;

    public function __construct(RadioBasiliskUtility $utility, array $options)
    {
    ...
    public function get_utility(): RadioBasiliskUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RadioBasiliskContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RadioBasiliskContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RadioBasiliskContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RadioBasiliskConfig::make_feature($name);
    ...
    private static function buildUrl(RadioBasiliskSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RadioBasiliskError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RadioBasiliskResponse($fetched);
        }
    ...
    private function fail(RadioBasiliskContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RadioBasiliskContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
