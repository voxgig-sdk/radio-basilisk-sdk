...
//    only - a typed RadioBasiliskContext there would make every ctx
...
final class RadioBasiliskOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new RadioBasiliskContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new RadioBasiliskSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new RadioBasiliskResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new RadioBasiliskError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new RadioBasiliskResponse($clean['response']);
    ...
    public static function err_from_map($m): ?RadioBasiliskError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new RadioBasiliskError(is_string($code) ? $code : '', $msg);
    }
