...
class MusicEntity
{
    ...
    {
        if ($args) {
            $this->_data = RadioBasiliskHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = RadioBasiliskHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   the SDK boundary; throws RadioBasiliskError on failure (item-5 convention).
     */
